...
Contributions are welcome. `go-ruby-tsort/tsort` is built to a small set of non-negotiable
...
- **Differential testing against MRI.** Correctness is defined by reference
  Ruby. A corpus of digraphs is sorted through both `ruby` (`TSort.tsort`,
  `TSort.strongly_connected_components`, `each_strongly_connected_component_from`)
  and this library and the results — including `TSort::Cyclic` messages — are
...
The library is in
[`github.com/go-ruby-tsort/tsort`](https://github.com/go-ruby-tsort/tsort). This documentation site is in
[`github.com/go-ruby-tsort/docs`](https://github.com/go-ruby-tsort/docs). Start from the
