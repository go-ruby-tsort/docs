#!/usr/bin/env bash
#
# Copyright (c) the go-ruby-tsort authors
# SPDX-License-Identifier: BSD-3-Clause
#
# Library-level cross-runtime benchmark runner.
#
# Runs the SAME workload through (a) the pure-Go go-ruby-tsort library
# (benchmarks/go) and (b) each available reference Ruby runtime
# (benchmarks/ruby/tsort.rb), then prints one Markdown table per sub-benchmark:
# ns/op and the ratio vs MRI.
#
# Before any timing it runs a VERIFY gate: the Go library's canonical output
# (the full topological order and the SCC partition) must be byte-identical to
# MRI's; a mismatch aborts, so no number is ever reported for a wrong result.
#
...
MOD=$(basename "$RB" .rb)
TMP=$(mktemp)
trap 'rm -f "$TMP" "$TMP.go" "$TMP.mri"' EXIT

# --- Correctness gate: Go library output must equal MRI, byte for byte. --------
echo "== verifying go-ruby-$MOD output == MRI ==" >&2
( cd go && go run . verify ) > "$TMP.go" 2>/dev/null
"$RUBY" "$RB" verify > "$TMP.mri" 2>/dev/null
if ! diff -q "$TMP.go" "$TMP.mri" >/dev/null; then
  echo "  FAIL: go output differs from MRI — aborting before timing." >&2
  diff "$TMP.go" "$TMP.mri" >&2 || true
  exit 1
fi
echo "  ok: topological order and SCC partition byte-identical to MRI" >&2
